...
System.register(['angular', 'jquery', 'moment', 'lodash', './bower_components/d3/d3.js', './gradient'], function (_export, _context) {
    "use strict";

    var angular, $, moment, _, d3;
    ...
    return {
        setters: [function (_angular) {
            ...
            _ = _lodash.default;
        }, function (_bower_componentsD3D3Js) {
            d3 = _bower_componentsD3D3Js.default;
        }, function (_gradient) {}],
        execute: function () {
            ...
            angular.module('grafana.directives').directive('grafanaWindRose', function ($rootScope, timeSrv, Rainbow) {
                return {
                    ...
                    link: function link(scope, elem) {
                        ...
                        function initSvg() {
                            svg = d3.select(elem[0]).append('svg').attr('id', 'chart').attr('width', containerW).attr('height', containerH).attr('viewBox', '0 0 1000 720').attr('preserveAspectRatio', 'xMidYMid meet').append('g').attr('transform', 'translate(480, 360)');

                            svg.style('display', 'none');

                            circles_container = svg.append('g').attr('class', 'circles_container');

                            axis_container = svg.append('g').attr('class', 'axis_container');

                            compass_rose_container = svg.append('g').attr('class', 'compass_rose_container').style('fill', 'none');

                            percents_scale_container = svg.append('g').attr('class', 'percents_scale_container');

                            line_tooltip_container = svg.append('g').attr('class', 'line_tooltip_container');

                            line_tooltip = line_tooltip_container.append('line').attr('class', 'line_tooltip').style('stroke', '#f00').style('opacity', 0);

                            line_tooltip_container.append('circle').attr('r', radius).attr('cx', 0).attr('cy', 0).attr('opacity', 0).attr('class', 'show_tooltip');

                            curve_container = svg.append('g').attr('class', 'curve_container');

                            points_container = curve_container.append('g').attr('class', 'points_container sortable');

                            chart_container = svg.append('g').attr('class', 'chart_container');
                        ...
                        function defineBg() {
                            ...
                            bgDefEl.append('pattern').attr('width', function (d) {
                                return d;
                                ...
                                return -1 * d / 2;
                            }).attr("id", bgId).attr('patternUnits', 'userSpaceOnUse').append("image").attr("xlink:href", "public/plugins/n-wind-rose-panel/img/windrose.svg").attr('width', function (d) {
                                return d;
                            }).attr('height', function (d) {
                                return d;
                            }).attr('x', 0).attr('y', 0);
                        ...
                        function defineTooltip() {
                            svg.selectAll('.show_tooltip').on('mousemove', function (d) {
                                if (peaksData) {
                                    ...
                                    var pointEl = line_tooltip_container.selectAll('.point_tooltip').data(tooltip_points);

                                    pointEl.enter().append('circle');
                                    pointEl.exit().remove();

                                    pointEl.attr('r', function (d) {
                                        return +d.point_radius + 3;
                                    }).attr('cx', function (d) {
                                        return d.x;
                                    }).attr('cy', function (d) {
                                        return d.y;
                                    }).attr('class', 'point_tooltip').style('fill', '#fff').style('stroke', '#f00');
                                    ...
                                    line_tooltip.attr('x1', 0).attr('x2', x2).attr('y1', 0).attr('y2', y2);
                                    //tooltip
                                    div.style('opacity', 1);

                                    var html = '<div class="degree">' + alpha + '\xB0 </div>';
                                    ...
                                    div.html(html).style('left', d3.event.pageX + 15 + 'px').style('top', d3.event.pageY - 38 + 'px');
                                }
                            }).on('mouseout', function () {
                                line_tooltip.attr('x1', 0).attr('x2', 0).attr('y1', 0).attr('y2', 0).style('opacity', 0);

                                var pointEl = line_tooltip_container.selectAll('.point_tooltip').data([]);

                                pointEl.enter().append('circle');
                                pointEl.exit().remove();

                                //tooltip
                                div.style('opacity', 0).style('left', 0).style('top', 0);
                            });
                        }
                        ...
                        function buildScaleContainer() {
                            ...
                            scale_container = svg.append('g').attr('class', 'scale_container').attr('transform', 'translate(-460, 0)');

                            scale_container.append('text').attr('x', -50).attr('y', 0).text(axisName).attr('transform', 'rotate(270, 0, 0)').attr('class', 'axis_title').style('fill', '#fff');

                            points_scale = scale_container.append('g').attr('class', 'points_scale');

                            chart_scale = scale_container.append('g').attr('class', 'chart_scale');

                            colored_scale = svg.append('g').attr('class', 'colored_scale').attr('transform', 'translate(370, 0)');

                            points_scale.append('line').attr('x1', lineX).attr('y1', -1 * radius).attr('x2', lineX).attr('y2', radius).style('stroke', '#fff').style('stroke-width', '5px');
                        }

                        function setAxisName() {
                            var axisName = options.axisName;
                            var titleEl = scale_container.selectAll('.axis_title').data([axisName]);
                            titleEl.enter().append('text');
                            titleEl.exit().remove();

                            titleEl.attr('x', -50).attr('y', 0).text(function (d) {
                                return d;
                            }).attr('transform', 'rotate(270, 0, 0)').attr('class', 'axis_title').style('fill', '#fff');
                        }
                        ...
                        function buildChartScale(numOfBlocks) {
                            ...
                            var chartScaleEl = chart_scale.selectAll('.chart_block').data(d);

                            chartScaleEl.enter().append('line');
                            chartScaleEl.exit().remove();

                            chartScaleEl.attr('x1', lineX).attr('y1', function (d) {
                                return d.y1;
                            }).attr('x2', lineX).attr('y2', function (d) {
                                return d.y2;
                            }).attr('class', 'chart_block').style('stroke', function (d) {
                                return d.color;
                        ...
                        function buildChartScaleLabels(step, numOfBlocks, percents) {
                            ...
                            var chartLabelEl = chart_scale.selectAll('.chart_label').data(d);

                            chartLabelEl.enter().append('text');
                            chartLabelEl.exit().remove();

                            chartLabelEl.attr('x', x).attr('y', function (d) {
                                return d.y;
                            }).attr('class', 'chart_label').text(function (d) {
                                return d.text;
                        ...
                        function buildScale() {
                            ...
                            var createStrokes = function createStrokes(ticks) {
                            ...
                            var strokesEl = points_scale.selectAll('.stroke').data(strokes);

                            strokesEl.enter().append('line');
                            strokesEl.exit().remove();

                            strokesEl.attr('x1', function (d) {
                                return d.x1;
                            }).attr('y1', function (d) {
                                return d.y1;
                            }).attr('x2', function (d) {
                                ...
                                return d.y2;
                            }).attr('class', 'stroke').style('stroke', '#fff').style('stroke-width', '3px');
                        }
                        ...
                        function buildColoredScale() {
                            ...
                            var coloredScaleEl = colored_scale.selectAll('.colored_scale_block').data(d);

                            coloredScaleEl.enter().append('line');
                            coloredScaleEl.exit().remove();

                            coloredScaleEl.attr('x1', lineX).attr('y1', function (d) {
                                return d.y1;
                            }).attr('x2', lineX).attr('y2', function (d) {
                                return d.y2;
                            }).attr('class', 'colored_scale_block').style('stroke', function (d) {
                                return d.color;
                            }).style('stroke-width', '12px');
                        }
                        ...
                        function buildColoredScaleLabels() {
                            ...
                            var coloredScaleLabelEl = colored_scale.selectAll('.colored_scale_label').data(d);

                            coloredScaleLabelEl.enter().append('text');
                            coloredScaleLabelEl.exit().remove();

                            coloredScaleLabelEl.attr('x', x).attr('y', function (d) {
                                return d.y;
                            }).attr('class', 'colored_scale_label').text(function (d) {
                                return d.text;
                            }).style('fill', '#fff').style('font-size', '13px');

                            var coloredScaleTitleEl = colored_scale.selectAll('.colored_scale_title').data([title]);

                            coloredScaleTitleEl.enter().append('text');
                            coloredScaleTitleEl.exit().remove();

                            coloredScaleTitleEl.attr('x', -50).attr('y', 0).attr('class', 'colored_scale_title').text(function (d) {
                                return d;
                            }).attr('transform', 'rotate(270, 0, 0)').style('fill', '#fff');
                        }

                        function buildR(r) {
                            var circleEl = circles_container.selectAll('.circle').data(r);

                            circleEl.enter().append('circle');
                            circleEl.exit().remove();

                            circleEl.attr('r', function (d) {
                                return d;
                            }).attr('class', 'circle').style('fill', '#fff').style('stroke', '#dcdcdc');
                        }

                        function buildCircumferences() {
                            var r = [];
                            var createR = function createR(ticks) {
                        ...
                        function buildPercentsScale(num, percents, alpha) {
                            ...
                            var percentsScaleEl = percents_scale_container.selectAll('.percents').data(d);

                            percentsScaleEl.enter().append('text');
                            percentsScaleEl.exit().remove();

                            percentsScaleEl.attr('x', function (d) {
                                return d.x;
                            }).attr('y', function (d) {
                                return d.y;
                            }).text(function (d) {
                                return d.text;
                            }).attr('class', 'percents').style('fill', '#a0a0a0').style('text-anchor', function (d) {
                                return d.textAnchor;
                        ...
                        function buildAxis() {
                            ...
                            var axisEl = axis_container.selectAll('.line').data(axis);

                            axisEl.enter().append('line');
                            axisEl.exit().remove();

                            axisEl.attr('x1', function (d) {
                                return d.x1;
                            }).attr('y1', function (d) {
                                return d.y1;
                            }).attr('x2', function (d) {
                        ...
                        function buildStrokesLabels() {
                            ...
                            var createLabels = function createLabels(labelTicks) {
                            ...
                            var labelEl = points_scale.selectAll('.axis-label').data(labels);

                            labelEl.enter().append('text');
                            labelEl.exit().remove();

                            labelEl.attr('x', function (d) {
                                return d.x;
                            }).attr('y', function (d) {
                                return d.y;
                            }).attr('class', 'axis-label').text(function (d) {
                                return d.text;
                            }).style('fill', '#fff').style('font-size', '14px').style('text-anchor', 'end');
                        }

                        function buildPoints(points) {
                            var circleEl = points_container.selectAll('.circle').data(points);

                            circleEl.enter().append('circle');
                            circleEl.exit().remove();

                            circleEl.attr('cx', function (d) {
                                return d.x;
                            }).attr('cy', function (d) {
                                return d.y;
                            }).attr('r', function (d) {
                                ...
                                return d.color;
                            }).attr('class', 'circle').on('mouseover', function (d) {
                                div.style('opacity', 1);
                        ...
                        function buildChart(d) {
                            var arcEl = chart_container.selectAll('.arc').data(d);

                            arcEl.enter().append('path');
                            arcEl.exit().remove();

                            arcEl.attr('d', function (d) {
                                return d.arc();
                            }).attr('class', 'arc').on('mouseover', function (d) {
                                div.style('opacity', 1);

                                var html = '<div class="angle">' + d.startAngle + '\xB0  -  ' + d.endAngle + '\xB0</div>';
                        ...
                        function buildExtraLines() {
                            ...
                            var curveEl = curve_container.selectAll('.peak_line').data(lines_data);

                            curveEl.enter().append('path');
                            curveEl.exit().remove();

                            curveEl.attr('d', function (d) {
                                return lineFunc(d.curve);
                            }).attr('stroke', function (d) {
                                return d.curve[0]['color'];
                            }).attr('stroke-width', function (d) {
                                return d.curve[0]['line_width'];
                            }).attr('fill', function (d) {
                                return d.curve[0]['color'];
                            }).attr('fill-opacity', function (d) {
                                return d.curve[0]['fill'];
                            }).attr('class', 'peak_line sortable show_tooltip');

                            var curveElPointsContainer = curve_container.selectAll('.peak_points_container').data(points_data);
                            ...
                            var curveElPoints = curveElPointsContainer.selectAll('.peak_points').data(function (d) {
                                return d.curve;
                            });
                            ...
                            curveElPoints.attr('cx', function (d) {
                                return d.x;
                            }).attr('cy', function (d) {
                                return d.y;
                            }).attr('r', function (d) {
                            ...
                            var curveElBarsContainer = curve_container.selectAll('.peak_bars_container').data(bars_data);
                            ...
                            var curveElBars = curveElBarsContainer.selectAll('.peak_bar').data(function (d) {
                                return d.curve;
                            });
                            ...
                            curveElBars.attr('x1', function (d) {
                                return d.x;
                            }).attr('y1', function (d) {
                                return d.y;
                            }).attr('x2', 0).attr('y2', 0).attr('stroke', function (d) {
                                return d.color;
                            }).attr('stroke-width', function (d) {
                                return d.line_width;
                        ...
                        function addCompassRoseAsBackground() {
                            ...
                            setTimeout(function () {
                                if (compassRose) {
                                    svg.select('.bg_container').attr('fill', 'url(#' + bgId + ')');
                                } else {
                                    svg.select('.bg_container').attr('fill', 'none');
                                }
                            }, 500);
                        }
                        ...
                        function setCompassRoseOpacity() {
                            ...
                            svg.select('.bg_container').style('opacity', function (d) {
                                return compassRoseOpacity;
                            });
                        }
                        ...
                        function styleAxisLabels() {
                            ...
                            var getAngleCoords = function getAngleCoords(init_alpha, delta) {
                            ...
                            if (degrees) {
                                for (var i = 0; i < 360 / alpha; i++) {
                                    ...
                                    labels.push({ x: coords.x, y: coords.y, text: init_alpha + '\xB0' });
                                }
                            }
                            if (cardinalCompass) {
                            ...
                            var axisStyleEl = svg.selectAll('.axis-style').data(labels);

                            axisStyleEl.enter().append('text');
                            axisStyleEl.exit().remove();

                            axisStyleEl.attr('x', function (d) {
                                return d.x;
                            }).attr('y', function (d) {
                                return d.y;
                            }).text(function (d) {
                                return d.text;
                            }).attr('fill', '#fff').attr('class', 'axis-style').attr('startOffset', '100%').style('text-anchor', 'middle').style('font-size', function (d) {
                                return d.fontSize || '14px';
                            });
                        }
                        ...
                        function renderExtraLines(preparedSpeeds) {
                            ...
                            peaksData = _.map(res, function (data, key) {
                                ...
                                j++;
                                var key_overrides = _.find(overrides, function (item) {
                        ...
                        function renderChart() {
                            ...
                            _.each(res, function (segment, i) {
                                ...
                                _.each(innerSegments, function (innerSegment, v) {
                                    ...
                                    var arc = d3.svg.arc().innerRadius(innerR).outerRadius(innerSegmentOuterR).startAngle(startAngle).endAngle(endAngle);
...
//# sourceMappingURL=wind_rose.js.map
