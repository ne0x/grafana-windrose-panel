...
System.register([], function (_export, _context) {
  ...
  return {
    setters: [],
    execute: function () {
      angular.module('grafana.services').factory('helper', function () {
        return {
      ...
      angular.module('grafana.services').factory('Color', ['helper', function (helper) {
        return {
          value: {
            h: 1,
            s: 1,
      ...
      angular.module('grafana.directives').directive('colorpicker', ['$document', '$compile', 'Color', 'helper', function ($document, $compile, Color, helper) {
        return {
          require: '?ngModel',
          restrict: 'A',
          link: function link($scope, elem, attrs, ngModel) {
            var template = '<div class="colorpicker dropdown-menu">' + '<div class="colorpicker-saturation"><i><b></b></i></div>' + '<div class="colorpicker-hue"><i></i></div>' + '<div class="colorpicker-alpha"><i></i></div>' + '<div class="colorpicker-color"><div></div></div>' + '<button class="close close-colorpicker">&times;</button>' + '</div>',
            ...
            var previewColor = function previewColor() {
              try {
            ...
            var slidersUpdate = function slidersUpdate(event) {
            ...
            var mousemove = function mousemove(event) {
              ...
              var left = Math.max(0, Math.min(slider.maxLeft, slider.left + ((event.pageX || pointer.left) - pointer.left)));

              var top = Math.max(0, Math.min(slider.maxTop, slider.top + ((event.pageY || pointer.top) - pointer.top)));
            ...
            var getColorpickerTemplatePosition = function getColorpickerTemplatePosition() {
              var positionValue,
            ...
            var hideColorpickerTemplate = function hideColorpickerTemplate() {
